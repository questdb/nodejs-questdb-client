---
name: review-pr
description: Review a GitHub pull request or local Git range against the QuestDB JavaScript client TypeScript ILP/QWP coding standards. Use when asked to review a PR or a local diff.
---

# Review a Node.js client pull request

**Usage:** `/skill:review-pr [PR number or URL | --range=<base>..<head>] [--level=0..3]`

Review the PR or local range identified by the invocation arguments. Pi appends
arguments after `/skill:review-pr` as a user request; treat that request text as
the review arguments. When invoked by name in ordinary chat, parse the user's
request the same way. Parse exactly one review target: a PR number/URL, or
`--range=<base>..<head>`. The range head may be omitted (`--range=<base>..`) to
...
Use Pi's `bash` for read-only `gh` and Git queries, repository searches (`rg`,
`rg --files`), and evidence-driven validation commands. Use `read` to inspect
files. Do not edit the primary working tree, push, post comments, or mutate the PR.
Step 3b may create isolated temporary worktrees for head/base execution or to
verify a regression test against reverted production hunks; remove them afterward.
Run validation that writes files only in a disposable worktree. Temporary review
artifacts must stay outside the repository. Do not use Pi's `edit` or `write`
tools on the project during a review.
...
Parse the review arguments for `--level=N`, `-lN`, or a bare digit `0`-`3`.
Default to level 0. Strip the level token and any `--range=` token before passing
a PR target to `gh`.

| Level           | What runs                                                                                                                                                                                                                                                                                                                                                |
| --------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **0 (default)** | Steps 1, 2, 2.4, 2.5f, 2.6, and 4. Discover issues inline without subagents. Build a compact coverage map and apply the Step 3b admission gate inline from a blank evidence form.                                                                                                                                                                              |
| **1**           | Add Steps 2.5a and 2.5e when tests change. Run Role 1 plus at most two applicable roles from Roles 2-7, 9-13, and 14-15. Independently falsify each surviving atomic candidate.                                                                                                                                                                        |
| **2**           | Run all of Step 2.5, restricting 2.5b to exported/public/protected symbols, transport interfaces, shared helpers, and configuration options. Run Role 1 plus at most four change-relevant roles. Independently falsify each surviving candidate.                                                                                                        |
| **3**           | Run the full workflow. Select at most six applicable discovery roles: Role 1 always; Role 8 when changed symbols have out-of-diff callers; Roles 2-7 and 14-15 when their domains are touched; Roles 9-13 for changed tests or a fix claim; Role 10 only when a distinct adversarial pass is warranted. Depth comes from evidence, not agent count. |
...
## Pi review delegation (levels 1-3)

An explicit user request for level 1, 2, or 3 authorizes the corresponding review
fanout. A default level 0 review stays inline; complexity alone never authorizes
subagents. Before delegating, read the installed `pi-subagents` skill and its
review/execution guidance. Call `subagents_enable`, then use Pi's `subagent` tool
with fresh-context `reviewer` children and an explicit read-only task boundary.
Use `workflowScript` with `runs.all` for a bounded discovery wave; give each child
a stable key and short label. A single bounded falsifier can use a direct
`{ agent: "reviewer", task: "...", context: "fresh", async: true }` launch.
Prefer async runs and consume their results before admission; Pi notifies the
parent on completion. Never treat a reviewer response as proof or authorization
to modify the PR. Do not replace a failed subagent run with an unannounced CLI
agent or claim that the requested level completed; report the blocker.

Steps 3 and 3b use fresh-context, read-only reviewer tasks. Give each child the
repository/cwd, revision identities, relevant artifacts, goal, scope, authority
boundary, evidence requirements, and concise output contract. Discovery tasks
receive the diff, Step 2.4 gitlink verdicts, the Step 2.5 surface map, the Step 2.6
coverage map, the chosen role, and the candidate contract. Role 10 and Step 3b
falsifiers are deliberate reduced-context exceptions. Do not give a falsifier
the discovery map or parent review narrative. No child delegates further.

Put parent-produced maps in temporary files outside the repository. For child
reports, use runtime-managed relative `output` paths instead of writing into
the repository or pasting large maps into every prompt. Never pass a discovery
narrative, proposed severity/fix, votes, or verification claims to a falsifier. The parent owns role selection, the private candidate ledger,
...
Use `rg` and `rg --files` via `bash` rather than reasoning about callers from
memory. The resulting map is input to every normal Step 3 reviewer.
...
Put the relevant facts at the top of normal Step 3 prompts. Role 10 receives only
...
Use fresh-context, read-only Pi `reviewer` tasks for the selected roles. Select only
roles materially touched by the change and obey the review-level cap. Reviewer
count is never evidence.
...
**Role 1 — Correctness and ILP semantics:** Check nullish omission, separators,
...
**Role 2 — Buffer and byte-encoding safety:** Reconstruct bytes and capacity math.
...
**Role 3 — Transport, negotiation, auth, and TLS:** Check serializer negotiation,
...
**Role 4 — Async, concurrency, and flush semantics:** Check every Promise/`await`,
...
**Role 5 — Resource management and lifecycle:** Trace sockets, Undici pools/agents,
...
**Role 6 — Performance and algorithmic optimality:** For each loop, scan, allocation,
...
**Role 7 — Public API, compatibility, and code quality:** Check both package roots,
...
**Role 8 — Cross-context caller impact:** Walk every 2.5b callsite with callers up to
...
**Role 9 — Test coverage:** Recheck every Step 2.6 test and failure link, add missed
...
**Role 10 — Fresh-context adversarial:** Receive only the diff and changed filenames.
...
**Role 11 — Test efficacy and correctness:** Trace each changed test from production
...
**Role 12 — Test-code quality:** Search the 2.5e inventory before flagging duplicated
...
**Role 13 — Regression-test efficacy:** For a bug-fix claim, identify which production
...
**Role 14 — QWP wire format and protocol sessions:** Reconstruct frame headers,
...
**Role 15 — Store-and-forward, replay, and failover:** Verify the durability contract
...
At levels 1-3, launch one fresh-context Pi `reviewer` falsifier per atomic candidate.
Give it only:
