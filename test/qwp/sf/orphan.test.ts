...
function collectIntValues(frames: Buffer[]): number[] {
  ...
  for (const f of frames) {
    ...
    let o = 12;
    if (flags & 0x08) {
      const a = readVarint(f, o); o = a.offset;
      const b = readVarint(f, o); o = b.offset;
      for (let i = 0; i < b.value; i++) {
        const r = readVarint(f, o); o = r.offset + r.value;
      }
    }
    for (let t = 0; t < tableCount; t++) {
      const nm = readVarint(f, o); o = nm.offset + nm.value;
      const rc = readVarint(f, o); o = rc.offset;
      const cc = readVarint(f, o); o = cc.offset;
      ...
      for (const cs of specs) {
        ...
        if (cs.name === "i" && cs.type === TYPE_LONG) {
          for (let r = 0; r < rc.value; r++) {
            out.push(Number(f.readBigInt64LE(o))); o += 8;
          }
        } else {
          break;
        }
...
async function orphanSlot(sfDir: string, serverPort: number, senderId: string): Promise<void> {
  ...
  for (let i = 0; i < 50; i++) {
    await producer.table("crash_t").intColumn("i", i).at(BigInt(1_700_000_000_000_000 + i), "us");
  }
...
function isCatchUpFrame(f: Buffer): boolean {
  if (f.length < 12 || f.subarray(0, 4).toString("ascii") !== "QWP1") return false;
  const flags = f.readUInt8(5);
  const tableCount = f.readUInt16LE(6);
...
describe("orphan scanner (spec 8.4)", () => {
  it("finds slots with a stale/absent lock and skips live-held ones", async () => {
    ...
    try {
      // dotfiles (.slot-locks area) and quarantined slots are not orphans
      mkdirSync(join(d, ".slot-locks", "x"), { recursive: true });
      mkdirSync(join(d, "def.quarantined.0"), { recursive: true });
      const orphans = await scanOrphans(d).then((o) => o.map((x) => x.senderId).sort());
...
describe("orphan drainer (spec 8.4)", () => {
  ...
  it("re-registers the recovered symbol dictionary before replaying a delta slot", async () => {
    ...
    const firstDataIdx = ack.frames.findIndex((f) => collectIntValues([f]).length > 0);
