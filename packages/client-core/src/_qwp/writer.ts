...
// The public packages emit several entry bundles, so a module-private brand
// would differ between the bundle that stamps a column and the bundle that
// validates it:
// a schema built with one emitted copy of the factories would be rejected by a
// writer built from another copy. The key carries a version
